...
function searchLocalMessages(messages: IMessage[], query: string): string {
  ...
  // 1. Strip out specific phrase patterns first
  ...
  // 2. Define common filler words to ignore so we focus on the core meaning
  const fillerWords = [
    'what', 'was', 'discussed', 'about', 'the', 'of', 'in', 'is', 
    'for', 'a', 'an', 'there', 'any', 'mention', 'who', 'to', 'join'
  ];

  // 3. Extract meaningful keywords from the stripped coreKeyword
  const searchTerms = coreKeyword
    .replace(/[^\w\s]/g, '') // Remove remaining punctuation
    .split(/\s+/)            // Split into an array of individual words
    .filter(word => word.length > 2 && !fillerWords.includes(word));

  // If filtering leaves nothing, fallback to the stripped coreKeyword
  if (searchTerms.length === 0) {
    searchTerms.push(coreKeyword);
  }

  // 4. Filter messages if they contain ANY of the target search terms
  const matches = messages.filter(m => {
    const msgTextLower = m.text.toLowerCase();
    const nicknameLower = m.nickname.toLowerCase();
    
    return searchTerms.some(term => 
      msgTextLower.includes(term) || nicknameLower.includes(term)
    );
  });
  
  // 5. Return results or friendly fallback message
  if (matches.length === 0) {
    // Join terms back up to show the user what words were attempted
    const searchAttempt = searchTerms.join(', ');
    return `🔍 [AI Local Search Mode]\nI scanned the history for keywords related to "${searchAttempt}" but found no direct matching mentions.`;
  }
